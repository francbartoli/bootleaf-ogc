...
OGC.MapLayer = function(ogc,module,config){
    this.OGC = ogc;

    // Copy all non-empty variables
    this._config = {};
    for(var x in config){
        if(!this.OGC.util.isEmpty(config[x])){
...
OGC.MapLayer.prototype = {
    ...
    */
    _makeLeafletLayer:function(){
        ...
        switch(this._config.layer_type){
            ...
            case 'WMS':
                ...
                if(!this.OGC.util.isEmpty(this._config.cql_filter)){
                    conf.cql_filter = this._config.cql_filter;
                }
                if(!this.OGC.util.isEmpty(this._config.styles)){
    ...
    hide:function(){
        if(this._leafletLayer !== false){
            this.OGC.map.removeLayers([this._leafletLayer]);
        }
        return true;
    },
    ...
    getFeatureInfo:function(e){
        ...
        var params = {
            ...
            bbox : this.OGC.map.map.getBounds().toBBoxString(),
            feature_count : 10,
            height : this.OGC.map.map.getSize().y,
            width : this.OGC.map.map.getSize().x,
            format : "image/png",
            info_format : "text/html",
            srs : "EPSG:4326",
            x : Math.round(this.OGC.map.map.layerPointToContainerPoint(e.layerPoint).x),
            y : Math.round(this.OGC.map.map.layerPointToContainerPoint(e.layerPoint).y),
            buffer : "8"
        };
        ...
        promise.then(
            function(res){
                ...
                if(self.OGC.util.isEmpty(res[0])){
                ...
                self.OGC.map.map.openPopup(popup);
            },
            function(failure){
