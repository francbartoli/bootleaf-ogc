...
OGC.Module.prototype = {
    ...
    loadModuleData:function(){
        ...
        if(this.ready.state() === 'resolved'){
            return self.OGC.util.fakePromise(self);
        }

        return $.getJSON(this.OGC.baseUrl + 'login/changeModule/' + this.appinst + '/' + this.module_id).then(
            function(success){
    ...
    _activate: function(self){
        ...
        self.OGC.map.setConfig(self.mapConfig);
        self.OGC.map.removeLayers(layersToRemove);
        self.OGC.appInst = self.appinst;
        self.OGC.user.activeModule = self;
        return self.OGC.util.fakePromise(self.name);
    },

    addLayers:function(layerConfigs){
        var layer;
        for(var i = 0;i<layerConfigs.length;i++){
            this.maplayers[layerConfigs[i].layer_id] = new OGC.MapLayer(this.OGC,this,layerConfigs[i]);
        }
    },
