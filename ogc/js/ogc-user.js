...
OGC.User = function(ogc){
    ...
    $('#loginModal').on({
        'shown.bs.modal': function(e){
            self.OGC.util.focusAtEnd('#username');
        },
    ...
    $('#username').on('keydown',this,function(e){
        if([13,32,186,188].indexOf(e.which) !== -1){
            self.OGC.util.focusAtEnd('#password');
        }
    });
...
OGC.User.prototype = {
    ...
    _handleAuthenticationSuccess: function(success){
        ...
        self.OGC.eventFilterLayer = success.eventFilterLayer;
        self.OGC.eventFilterName = success.eventFilterName;
        self.OGC.logo = success.logo;
        self.OGC.projectName = success.projectName;
        ...
        for(var i = 0;i<success.userModules.length;i++){
            self.modules[success.userModules[i].appinst] = new OGC.Module(self.OGC,success.userModules[i]); 
        }

        // Add extra data to current modules
        if(typeof self.modules[success.appInst] == 'undefined'){
            self.modules[success.appInst] = new OGC.Module(self.OGC,{
        ...
        if(typeof self.modules.master == 'undefined'){
            self.modules['master'] = new OGC.Module(self.OGC,{
                module_id: 99,
                name: 'Default',
                appinst: 'master'
            });
        ...
        if(masterPromise === false){
            promises = self.OGC.util.fakePromise(self);
        }else{
        ...
        promises = promises.then(function(worked){
                return self.OGC.map.useAppInst(success.appInst);
        });
    ...
    useAppInst:function(appinst){
        ...
        if(typeof self.modules[appinst] !== 'undefined'){
            return self.modules[appinst].activate().then(function(){
                ...
                self.OGC.appInst = appinst;
                self.OGC.map.useAppInst(self.OGC.appInst);
            });
        }else{
    ...
    logout:function(e){
        var self = e.data;
        $.post(self.OGC.baseUrl + 'login/logout').then(
            function(success){
